...
const SPAWN_EXPLOSION_SCENE: PackedScene = preload("res://Characters/Enemies/spaw_explosion.tscn")
...
@onready var tilemap: TileMapLayer = get_node("Bloquear")
...
func _close_entrance() -> void:
	for entry_position in entrance.get_children():
		tilemap.set_cell(tilemap.local_to_map(entry_position.position), 2, Vector2i.ZERO)
		tilemap.set_cell(tilemap.local_to_map(entry_position.position) + Vector2i.DOWN, 3, Vector2i.ZERO)
...
func _spawn_enemies() -> void:
	for enemy_position in enemy_positions_container.get_children():
		...
		if boss_room:
			...
			if randi() % 2 == 0:
				enemy = ENEMY_SCENES.FLYING_CREATURE.instantiate() # coloca ideitação se for colocar if
			else:
